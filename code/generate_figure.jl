...
# pick bigger matrices, ones that requires gb at least half the size of available memory. 
threshold = median(gemm_no_missing[:, :avail_mem_gb]) / 2
selected = gemm_no_missing[gemm_no_missing.req_mem_gb .> threshold, :]
...
speedup = collect(selected[:speedup]);
